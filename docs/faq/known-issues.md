...
| 좀비 공습 도중 랜덤 텔레포트가 발동하면 좀비가 적게 떨어짐 | 해결 (공습 중 랜덤 텔레포트 대기) |
| 강령술이 세이프하우스 안에서 그대로 발동함 | 해결 (설정으로 조절 가능, 기본은 대기) |
| 헬기·드론 화력지원이 끝난 뒤에도 로터음·총성이 계속 들림 | 해결 |
| 헬기 화력지원 중 소리와 사격은 나오는데 헬기 기체가 안 보임 | 해결 |
| 인벤토리 세이브 티켓으로 돌려받은 총에서 부착물이 빠짐 | 해결 |
| 강령술로 되살아난 좀비가 블러드문 중에도 뛰지 않음 | 해결 |
| 좀비 랜덤화 모드(BLT)와 함께 쓸 때 특수좀비 능력치가 덮어씌워짐 | 해결 |
| 좀비 위 표시(이름표 등)를 그릴 때 게임 오류가 반복됨 | 해결 |
| GunFighter 모드와 함께 쓸 때 좀비화된 플레이어를 때리면 오류 발생 | 해결 |
| 화면 카운터가 프레임(FPS)에 따라 빠르거나 느리게 흐름 | 해결 |
