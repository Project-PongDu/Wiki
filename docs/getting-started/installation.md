...
시즌에 참가하기로 한 서버의 디스코드에 입장하면 해당 채널에서 최신 `PongDu.exe`를 받을 수 있습니다. 한 번 받은 뒤의 업데이트는 런처가 자동으로 처리합니다.
...
런처를 실행할 때 새 버전이 있으면 **"새 버전이 있습니다"** 창이 뜹니다. 현재 버전과 새 버전, 변경 내용이 표시됩니다.

| 버튼 | 동작 |
| ---- | ---- |
| **지금 업데이트** | 새 버전을 받아 교체한 뒤 런처가 자동으로 다시 켜집니다 |
| **나중에** | 이번에는 기존 버전으로 실행합니다. 다음 실행 때 다시 안내합니다 |

로그인 정보와 설정은 유지되므로 다시 설정할 필요가 없습니다.

!!! warning "필수 업데이트"

    치지직 API 변경처럼 구버전이 더 이상 동작하지 않는 경우에는 **나중에** 대신 **종료** 버튼이 나옵니다. 업데이트해야 사용할 수 있습니다.

!!! note "v5.5.0 미만을 쓰고 있다면"

    자동 업데이트는 v5.5.0부터 들어갔습니다. 그 이전 버전이라면 디스코드에서 최신 `PongDu.exe`를 한 번 받아 덮어쓰세요. 이후로는 자동으로 갱신됩니다.
