...
서버 설정 `RT_Mode`로 고릅니다.

| 방식 | 동작 |
| ---- | ---- |
| **편도** (기본) | 최소~최대 거리 안의 무작위 좌표로 이동하고 끝 |
| **왕복** | 무작위 좌표로 이동한 뒤, 정해진 시간 동안 살아남으면 **원래 자리로 자동 복귀** |
| **랜덤 플레이어에게** | 접속 중인 다른 플레이어 한 명의 곁으로 이동 |

**랜덤 플레이어에게** 방식은 본인, 관리자 등 스태프, 사망한 플레이어, 세이프하우스 근처에 있는 플레이어를 대상에서 뺍니다. 보낼 대상이 없으면 **편도로 대신 발동**하므로 후원이 사라지지 않습니다.
...
도착 후 정해진 시간 동안 살아남으면 **원래 있던 자리로 자동 복귀**합니다. 남은 시간이 화면에 표시됩니다.

## 서버 설정

| 옵션 | 범위 | 기본값 | 설명 |
| ---- | ---- | ------ | ---- |
| `RT_Mode` | - | 편도 | 방식 (편도 / 왕복 / 랜덤 플레이어에게) |
| `RT_MinDist` | 10~1000 | 100 | 최소 이동 거리 (타일). 편도·왕복 |
| `RT_MaxDist` | 20~2000 | 200 | 최대 이동 거리 (타일). 편도·왕복 |
| `RT_SurviveMinutes` | 1~60 | 5 | 복귀까지 버텨야 하는 시간 (분). 왕복만 |
...
!!! info "대기하는 경우"
    ...
    - [좀비 공습](zombie-rain.md)이 진행 중일 때
